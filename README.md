...
$group-by=state&$group-by=city&$sum=pop&$sort=pop asc&$group-by(1)=state
&$last(biggestCity,1)=city&$last(biggestPop, 1)=pop-sum&$first(smallestCity, 1)=city
&$first(smallestPop, 1)=pop-sum</pre></code>
